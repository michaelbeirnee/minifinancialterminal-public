...
| **Signal research** | Independent cross-sectional OOS testing across price, volume, filing-lagged fundamentals, earnings/analyst events, archived estimates/options and trailing peer relationships, followed by sector/industry-neutral evidence, Benjamini-Hochberg false-discovery control, correlation-aware redundancy clustering, and execution-aware net-alpha/capacity gates using ADV, a historical high/low spread proxy, volatility and square-root impact. Current-only estimate/option data is captured into a point-in-time store instead of backfilled into history; the surviving blend also emits a current capacity-scaled neutral target. See [docs/signal-research.md](docs/signal-research.md). |
| **Production trading** | A deterministic daily cycle that consumes only frozen, approved research vintages: score, sleeve, risk-project, diff against broker positions, pass a hard risk gateway (kill switch included), write next-session orders to a ledger, reconcile fills. Ledger simulator by default; Alpaca paper-only adapter optional; see [docs/production-trading.md](docs/production-trading.md). |
